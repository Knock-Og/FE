...
const FindPwForm = () => {
  ...
  const pwSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!email || email.trim() === "") return setError("이메일을 적어주세요!");
    if (email.indexOf("@") === -1)
      return setError("'@'를 포함해서 이메일을 작성해주세요!");
    if (emailBoolean) return setError("이메일형식이 올바르지 않습니다!");
  ...
  const copyCode = () => {
    window.navigator.clipboard
      .writeText(copyPw)
      .then(() => setSuccess("임시비밀번호가 복사되었습니다."));
  };
