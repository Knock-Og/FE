...
const SignUpForm = () => {
  ...
  //직급셀렉부분
  const positionList = ["직급을 선택해주세요", "member", "manager", "owner"];
  ...
  const signupSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (memberName.trim() === "") return alert("이름을 확인해주세요!");
    if (position.trim() === "") return alert("직급을 선택해주세요");
    if (password.trim() === "") return alert("비밀번호를 확인해주세요!");
    if (passwordCheck.trim() === "")
      return alert("비밀번호 확인란을 선택해주세요");
    if (email.trim() === "") return alert("이메일을 선택해주세요");
    const signUpData = {
      position: position,
...
const StSignSelect = styled.select`
  ...
  border-bottom: 1px solid #ececec;
`;
const StCheckBtn = styled.button`position:absolute;right:0;`;
